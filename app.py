...
def page_command_center(exchange_id: str, symbol: str):
    ...
    if exchange_id=="binanceusdm": st.caption("DATA ROUTER · Price/candles/order book: official Binance Vision public market data (Spot proxy) · Funding/OI: real Binance Futures only when reachable · No synthetic financial values.")
    try:
...
def page_market_analysis(exchange_id: str, symbol: str):
    ...
    if exchange_id=="binanceusdm": st.caption("Source: Binance Vision public Spot market data for technical analysis/order book. Futures-only derivatives remain real-only and may show UNAVAILABLE if Binance Futures rejects the Streamlit host region.")
...
def page_scanner(exchange_id: str):
    ...
    if exchange_id=="binanceusdm":
        st.caption("ANALYSIS ROUTER · Ranking/velas/volumen desde Binance Vision public Spot como proxy de mercado. Funding/OI sólo se muestran cuando el endpoint real de Binance Futures responde; no se inventan valores.")
    c1,c2,c3=st.columns(3)
...
def sidebar() -> Tuple[str,str,str,str]:
    with st.sidebar:
        ...
        if exchange_id=="binanceusdm":
            st.caption("📡 Public data: Binance Vision (Spot proxy). Funding/OI: Binance Futures only when reachable. LIVE stays fail-closed and revalidates on the real futures API.")
