...
APP_VERSION = "2.0.2"
BUILD = "2026.09.15-PRO500-MENUCORE"
...
.stTabs [data-baseweb="tab-list"]{gap:6px;overflow-x:auto;scrollbar-width:none}.stTabs [data-baseweb="tab-list"]::-webkit-scrollbar{display:none}.stTabs [data-baseweb="tab"]{background:#0d1826;border:1px solid #1f334a;border-radius:10px;padding:8px 12px;white-space:nowrap}.stTabs [aria-selected="true"]{background:#13243a!important;border-color:#3a80a8!important}
...
@media (max-width:1280px){
 ...
 [data-testid="column"]{min-width:min(100%,320px)!important;flex:1 1 320px!important}
...
def page_market_analysis(exchange_id: str, symbol: str):
    ...
    with tabs[0]:
        ...
        with c1:
            ...
            else: st.dataframe(snap["levels"],use_container_width=True,hide_index=True,height=390)
        with c2:
            ...
            else: st.dataframe(snap["liquidity"],use_container_width=True,hide_index=True,height=390)
        ...
        st.dataframe(indicator,use_container_width=True,hide_index=True,height=320)
    with tabs[1]:
    ...
    with tabs[2]:
        ...
        if st.button(tr("Load recent trade flow"),key="flow_load"):
            try:
                if exchange_id=="binanceusdm":
                    recent_trades=_vision_recent_trades(symbol,500)
                else:
                    ad=ExchangeAdapter(exchange_id)
                    recent_trades=ad.fetch_recent_trades(symbol,500)
                    ad.close()
...
def sidebar() -> Tuple[str, str, str, str]:
    ...
    if hasattr(st, "segmented_control"):
        density = st.segmented_control("Vista / View", ["FOCUS", "PRO"], default=st.session_state.get("view_density", "PRO"), key="density_control")
    else:
        current_density = st.session_state.get("view_density", "PRO")
        density = st.selectbox("Vista / View", ["FOCUS", "PRO"], index=0 if current_density == "FOCUS" else 1, key="density_control_fallback")
    if density:
...
def main():
    ...
    if menu_visible:
        # App-owned two-column shell. Unlike st.sidebar, Safari/iPad cannot
        # independently collapse this column behind our back.
