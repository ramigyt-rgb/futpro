...
APP_VERSION = "2.0.3"
BUILD = "2026.09.15-PRO500-RESPONSIVEFIX"
...
def read_table(name: str) -> pd.DataFrame:
    allowed = {"journal", "audit", "capital_flows", "playbook", "alerts", "paper_positions", "paper_orders", "execution_intents", "execution_events", "market_snapshots"}
...
def init_state():
    ...
    defaults = {
        ...
        "timeframe": "15m", "debug_mode": False, "kill_switch": bool(persisted.get("kill_switch",False)),
...
def derivatives_engine(exchange_id: str, symbol: str, df: pd.DataFrame) -> Dict[str,Any]:
    ...
    oi_heat=clamp(abs(safe_float(result.get("oi_change_1h"),0))*8,0,35)
    result["crowding_score"]=clamp(fp*.55+oi_heat,0,100)
    result["crowding_status"]="EXTREME" if result["crowding_score"]>=80 else "HIGH" if result["crowding_score"]>=60 else "NORMAL"
    return result
...
def market_snapshot(exchange_id: str, symbol: str, timeframe: str="15m") -> Dict[str,Any]:
    ...
    flow=order_flow_from_trades(trades)
    ...
    snap={"symbol":symbol,"ticker":ticker,"orderbook":ob,"df":df,"derivatives":deriv,"order_flow":flow,"swings":swings,"structure":structure,"levels":levels,"liquidity":liq,"regime":reg,"confidence":conf,"integrity":integrity,"readiness":readiness,"confluence":confl,"scenarios":scenarios,"volatility":vr,"volume":ve,"last":last,"bid":bid,"ask":ask,"spread":spread,"spread_pct":spread_pct,"orderbook_quality":obq,"errors":errors,"status":"OK","latency_ms":(time.perf_counter()-started)*1000,"updated_at":iso_now(),"data_lineage":{"ticker":ticker.get("data_source","CCXT_FUTURES"),"candles":df.attrs.get("data_source","CCXT_FUTURES"),"derivatives":deriv.get("source","CCXT_FUTURES"),"order_flow":(flow.get("note") or flow.get("reason") or "recent trades")}}
...
def page_command_center(exchange_id: str, symbol: str):
    ...
    try:
        ...
        with c3: st.plotly_chart(gauge_chart(confl["no_trade"],"NO TRADE"),use_container_width=True,config={"displayModeBar":False})
        with c4: st.plotly_chart(gauge_chart(snap.get("readiness",{}).get("score",0),"READINESS"),use_container_width=True,config={"displayModeBar":False})
...
def page_market_analysis(exchange_id: str, symbol: str):
    ...
    with tabs[2]:
        ...
        if st.button(tr("Load recent trade flow"),key="flow_load"):
            try:
                recent_trades=cached_recent_trades(exchange_id,symbol,500)
                st.session_state["flow_result"]=order_flow_from_trades(recent_trades)
            except Exception as e:
                st.session_state["flow_result"]={"status":"UNAVAILABLE","reason":str(e)}
        flow=st.session_state.get("flow_result",{"status":"NOT LOADED"})
        flow_status=str(flow.get("status","NOT LOADED")).upper() if isinstance(flow,dict) else "UNAVAILABLE"
        if flow_status=="NOT LOADED":
            st.info("El flujo reciente todavía no fue cargado. Tocá ‘Cargar flujo reciente’ para analizar agresión compradora/vendedora." if st.session_state.get("language","es")=="es" else "Recent flow has not been loaded yet. Tap ‘Load recent trade flow’ to analyze buyer/seller aggression.")
        elif flow_status=="UNAVAILABLE":
            st.warning(("Flujo no disponible: " if st.session_state.get("language","es")=="es" else "Flow unavailable: ")+str(flow.get("reason","source unavailable")))
        else:
            fcols=st.columns(4)
            flow_cards=[
                ("IMBALANCE",fmt_pct(flow.get("imbalance")),"buyer vs seller aggression"),
                ("DELTA",fmt_num(flow.get("delta")),"observed classified volume"),
                ("AGGRESSION",f"{safe_float(flow.get('aggression_ratio'),0):.2f}×","buy/sell ratio"),
                ("SAMPLE",f"{int(safe_float(flow.get('sample_trades'),0))}","recent trades")
            ]
            for col,(title,value,sub) in zip(fcols,flow_cards):
                with col: render_card(title,value,sub)
            with st.expander("Detalle del flujo observado" if st.session_state.get("language","es")=="es" else "Observed flow detail",expanded=False):
                detail=pd.DataFrame([
                    {"Metric":"Buy volume","Value":fmt_num(flow.get("buy_volume"))},
                    {"Metric":"Sell volume","Value":fmt_num(flow.get("sell_volume"))},
                    {"Metric":"Avg trade size","Value":fmt_num(flow.get("avg_trade_size"))},
                    {"Metric":"Median trade size","Value":fmt_num(flow.get("median_trade_size"))},
                    {"Metric":"Large trade delta","Value":fmt_num(flow.get("large_trade_delta"))},
                    {"Metric":"Trades / second","Value":f"{safe_float(flow.get('trades_per_second'),0):.3f}"},
                ])
                render_dark_table(detail,max_rows=20)
...
def page_backtest(exchange_id: str, symbol: str):
    st.markdown(tr("## Backtest / Walk-Forward")); c1,c2,c3,c4=st.columns(4); tf=c1.selectbox(tr("Timeframe"),["5m","15m","1h","4h","1d"],index=2,key="bt_tf"); strategy=c2.selectbox(tr("Strategy"),["Trend Pullback","Breakout Retest","Range Reversal","Momentum Continuation"]); bars=c3.slider(tr("Bars"),300,1500,750,50); risk=c4.number_input(tr("Risk %"),.1,5.0,1.0,.1,key="bt_risk")
...
MAIN APP""",language="text")
    ...
    with tabs[5]:
        ...
        for table in ["journal","capital_flows","playbook","alerts","paper_positions","paper_orders","execution_intents","execution_events","market_snapshots"]:
...
def sidebar() -> Tuple[str, str, str, str]:
    ...
    remembered_tf = st.session_state.get("timeframe", "15m")
    if remembered_tf not in TIMEFRAMES:
        remembered_tf = "15m" if "15m" in TIMEFRAMES else TIMEFRAMES[0]
