...
.block-container{padding-top:1.2rem;padding-bottom:4rem;max-width:1800px;}
...
.pro-card.tight{padding:10px 12px}.pro-title{font-weight:800;letter-spacing:.06em;font-size:.78rem;color:#9fb0c6;text-transform:uppercase}.pro-value{font-size:1.55rem;font-weight:850;margin-top:5px;color:#f4f7fb}.pro-sub{font-size:.77rem;color:var(--muted);margin-top:3px}
...
def render_card(title: str, value: str, sub: str = "", tone: str = ""):
    st.markdown(f'<div class="pro-card {tone}"><div class="pro-title">{title}</div><div class="pro-value">{value}</div><div class="pro-sub">{sub}</div></div>', unsafe_allow_html=True)
...
def init_state():
    defaults = {
        ...
        "selected_nav": "Command Center", "paper_start_balance": 10000.0,
    }
...
def cached_derivatives(exchange_id: str, symbol: str) -> Dict[str, Any]:
    ad = ExchangeAdapter(exchange_id, private=False)
    try:
        return {"funding": ad.fetch_funding(symbol), "oi": ad.fetch_open_interest(symbol)}
    finally: ad.close()
...
def scan_market(exchange_id: str, max_symbols: int=15, timeframe: str="1h", min_quote_volume: float=5_000_000) -> pd.DataFrame:
    ...
    try:
        ...
        if not ranked:
            ranked=[(s,0,{}) for s in symbols[:max_symbols]]
        ...
        for sym,qv,t in ranked:
            try:
                ...
                rows.append({"Symbol":sym,"Last":safe_float(t.get("last"),r.close),"24h Vol":qv,"Trend":tr["trend"],"Structure":ss["trend"],"ADX":safe_float(r.adx),"RSI":safe_float(r.rsi),"ATR%":safe_float(r.atr_pct),"RVOL":safe_float(r.rvol),"Vol Regime":vr["regime"],"Funding %":funding,"Open Interest":oi,"Long Score":score_long,"Short Score":score_short,"Avoid Score":clamp(avoid,0,100),"Regime":reg.status})
            except Exception as e:
...
def render_issues(issues: List[ValidationIssue]):
    if not issues:
        st.success("No validation issues detected by the current rule set.")
        return
...
def render_decision(confl: Dict[str,Any], conf: EngineResult):
    ...
    if conf.score<45 or nt>=70: decision="NO TRADE"; cls="bad"
    elif max(long_s,short_s)<60: decision="WAIT"; cls="warn"
    else: decision="LONG BIAS" if long_s>short_s else "SHORT BIAS"; cls="good" if abs(long_s-short_s)>10 else "warn"
    st.markdown(f'<div class="pro-card"><div class="pro-title">DECISION MATRIX</div><div class="decision {cls}">{decision}</div><div class="pro-sub">Signal strength {max(long_s,short_s):.0f}/100 · Data confidence {conf.score:.0f}/100 · No-trade score {nt:.0f}/100</div></div>',unsafe_allow_html=True)
...
def app_header(mode: str, exchange_id: str, symbol: str):
    ...
    st.markdown(f'<div class="pro-card {tone}"><div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;align-items:center"><div><div style="font-size:1.45rem;font-weight:900;letter-spacing:.04em">⚡ {APP_NAME}</div><div class="pro-sub">Capital protection → Risk → Decision quality → Execution → Results</div></div><div>{html_badge(mode,"bad" if mode=="LIVE" else "warn" if mode=="PAPER" else "info")}{html_badge(SUPPORTED_EXCHANGES.get(exchange_id,exchange_id),"muted")}{html_badge(symbol,"info")}</div></div></div>',unsafe_allow_html=True)
...
def page_command_center(exchange_id: str, symbol: str):
    ...
    try:
        with st.spinner("Loading live public market data…"):
            snap=market_snapshot(exchange_id,symbol,"15m")
        if snap["status"]!="OK":
            if snap.get("error_code")=="HOST_REGION_RESTRICTED":
                st.markdown("<div class='pro-card mode-live'><div class='pro-title'>DEPLOYMENT REGION BLOCKED</div><div class='pro-value' style='font-size:1.25rem'>Exchange market data rejected by the hosting region</div><div class='pro-sub'>The app itself is running correctly. The selected exchange is refusing requests from the server that hosts this Streamlit deployment. Streamlit Community Cloud runs in the United States, so global futures APIs may reject the server even when you personally are located elsewhere.</div></div>",unsafe_allow_html=True)
                st.info("Safe options: switch to an exchange/data source that serves this host region, run the app locally, or deploy this same app on infrastructure in a jurisdiction supported by the exchange. The app will not attempt to bypass exchange geographic restrictions.")
            else:
                st.error("Market data is unavailable. The terminal remains operational in degraded mode.")
            if snap.get("errors"):
        ...
        ctxcols=st.columns(5)
        with ctxcols[0]: render_card("BTC 24H",fmt_pct(btc_ch),"market context")
        with ctxcols[1]: render_card("ETH 24H",fmt_pct(eth_ch),"market context")
        with ctxcols[2]: render_card("24H VOLUME",fmt_num(t.get("quoteVolume")),"quote volume if reported")
        with ctxcols[3]: render_card("RISK ENVIRONMENT",risk_env,"heuristic BTC/ETH + volatility context")
        with ctxcols[4]: render_card("DATA HEALTH",conf.status,f"{conf.score:.0f}/100")
        cols=st.columns(5)
        with cols[0]: render_card("MARKET REGIME",reg.status,f"confidence {reg.confidence:.0f}%")
        with cols[1]: render_card("VOLATILITY",vol["regime"],f"ATR percentile {fmt_num(vol.get('percentile'),0)}")
        with cols[2]: render_card("VOLUME",ve.get("status","—"),f"RVOL {fmt_num(ve.get('rvol'))}")
        with cols[3]: render_card("FUNDING",fmt_pct(safe_float(d.get("funding"))*100,4),d.get("funding_context","UNAVAILABLE"))
        with cols[4]: render_card("OPEN INTEREST",fmt_num(d.get("oi")),d.get("oi_context","UNAVAILABLE"))
        ...
        with c2:
            st.markdown("**Contradictions / risks**")
...
def page_market_analysis(exchange_id: str, symbol: str):
    ...
    tabs=st.tabs(["Chart & Structure","Multi-Timeframe","Derivatives & Flow","Scenarios","Universe"])
    ...
    if snap.get("status")!="OK":
        st.error("Selected symbol data unavailable. Change exchange/symbol or retry."); return
    df=snap["df"]
    with tabs[0]:
        ...
        with c1:
            ...
            if snap["levels"].empty: st.info("No levels detected.")
            else: st.dataframe(snap["levels"],use_container_width=True,hide_index=True)
        with c2:
            st.markdown("#### Liquidity map")
            st.caption("Levels are observed; the interpretation as stop/liquidity clusters is explicitly inferential.")
            if snap["liquidity"].empty: st.info("No clustered liquidity inference available.")
            else: st.dataframe(snap["liquidity"],use_container_width=True,hide_index=True)
        with c3:
            st.markdown("#### Indicator state")
            r=df.iloc[-1]
    ...
    with tabs[1]:
        chosen=st.multiselect("Timeframes",TIMEFRAMES,default=DEFAULT_MTF,key="mtf_select")
        if st.button("Run multi-timeframe engine",key="run_mtf"):
            with st.spinner("Analyzing selected timeframes…"): st.session_state["mtf_df"]=mtf_analysis(exchange_id,symbol,chosen)
        mtf=st.session_state.get("mtf_df",pd.DataFrame())
        if mtf.empty: st.info("Run the engine to avoid unnecessary API calls.")
        else:
    ...
    with tabs[2]:
        ...
        if st.button("Load recent trade flow",key="flow_load"):
            try:
                ad=ExchangeAdapter(exchange_id); tr=ad.fetch_recent_trades(symbol,500); ad.close(); st.session_state["flow_result"]=order_flow_from_trades(tr)
        ...
        st.caption("Aggressive buy/sell interpretation is only shown when the exchange's recent-trade feed includes a usable side. No synthetic CVD is presented as real order flow.")
    with tabs[3]:
        ...
        st.caption("Scenario scores are relative rule-based scores, not calibrated probabilities unless separately validated on historical out-of-sample data.")
    with tabs[4]:
        try:
            meta=cached_market_meta(exchange_id,symbol); st.json(meta,expanded=False)
            st.caption("Contract specifications are loaded from the exchange through CCXT rather than hard-coded.")
        except Exception as e: st.warning(f"Contract metadata unavailable: {e}")
        st.markdown("#### Optional external context")
        st.info("BTC dominance / TOTAL / TOTAL2 / TOTAL3 and economic calendar remain disabled unless a real external data source is explicitly integrated. The app does not invent these values.")

def page_scanner(exchange_id: str):
    st.markdown("## Futures Scanner")
    c1,c2,c3=st.columns(3)
    max_syms=c1.slider("Contracts to scan",5,30,12,1); tf=c2.selectbox("Scanner timeframe",["15m","1h","4h"],index=1); minvol=c3.number_input("Min 24h quote volume",min_value=0.0,value=5_000_000.0,step=1_000_000.0)
    if st.button("Run scanner",type="primary"):
        with st.spinner("Scanning liquid perpetual contracts with rate-limit-aware sequential requests…"):
    ...
    if s.empty: st.info("Run the scanner. It intentionally does not refetch on every widget change."); return
    filt=st.multiselect("Direction filter",["LONG CANDIDATES","SHORT CANDIDATES","AVOID"],default=["LONG CANDIDATES","SHORT CANDIDATES","AVOID"])
    x=s.copy()
    if "Long Score" in x:
    ...
    st.caption("Opportunity components are visible. This scanner ranks conditions; it does not assert that top-ranked trades will be profitable.")
...
def build_trade_inputs(exchange_id: str, symbol: str) -> Optional[Tuple[TradePlan,TradeCalc,Dict[str,Any]]]:
    ...
    with st.form("trade_builder_form"):
        c1,c2,c3,c4=st.columns(4)
        side=c1.selectbox("Direction",["LONG","SHORT"],index=0 if default_side=="LONG" else 1)
        order_type=c2.selectbox("Order type",["MARKET","LIMIT"])
        entry=c3.number_input("Entry",min_value=0.0,value=float(current),format="%.8f")
        lev=c4.number_input("Leverage",min_value=1.0,max_value=125.0,value=3.0,step=1.0)
        atr=safe_float(snap["df"].atr.iloc[-1],current*.01)
        default_stop=(entry-1.5*atr) if side=="LONG" else (entry+1.5*atr)
        c1,c2,c3,c4=st.columns(4)
        stop=c1.number_input("Stop loss",min_value=0.0,value=float(default_stop),format="%.8f")
        capital=c2.number_input("Capital / equity (USDT)",min_value=1.0,value=10000.0,step=100.0)
        risk_pct=c3.number_input("Max risk %",min_value=.01,max_value=100.0,value=float(get_risk_limits().max_risk_trade_pct),step=.1)
        setup=c4.selectbox("Setup",["Manual","Trend Pullback","Breakout Retest","Range Reversal","Liquidity Sweep","Momentum Continuation"])
        ...
        c1,c2,c3,c4=st.columns(4)
        maker=c1.number_input("Maker fee %",min_value=0.0,value=.02,step=.005,format="%.4f")
        taker=c2.number_input("Taker fee %",min_value=0.0,value=.05,step=.005,format="%.4f")
        slip=c3.number_input("Expected slippage % / side",min_value=0.0,value=.02,step=.01,format="%.4f")
        funding=c4.number_input("Expected funding cost %",min_value=-5.0,max_value=5.0,value=0.0,step=.01,format="%.4f")
        notes=st.text_area("Trade thesis / notes",placeholder="Trigger, invalidation, why this trade exists, what would make you stand down…")
        submitted=st.form_submit_button("Calculate & validate",type="primary")
...
def page_trade_builder(exchange_id: str, symbol: str, mode: str):
    st.markdown("## Trade Builder / Execution Gate")
    ...
    st.caption("Liquidation is an indicative isolated-linear estimate only. For existing LIVE positions, exchange-reported liquidation price takes precedence.")
    c1,c2=st.columns(2)
    with c1:
        st.markdown("#### Take-profit ladder")
        st.dataframe(pd.DataFrame(calc.tp_rows),use_container_width=True,hide_index=True)
    with c2:
        st.markdown("#### Stop alternatives")
    ...
    st.markdown("#### Psychology check")
    pc1,pc2,pc3,pc4=st.columns(4)
    fomo=pc1.slider("FOMO",0,10,0); revenge=pc2.slider("Revenge urge",0,10,0); fatigue=pc3.slider("Fatigue",0,10,0); impulsivity=pc4.slider("Impulsivity",0,10,0)
    psych=max(fomo,revenge,fatigue,impulsivity)
    if psych>=8: st.error("Behavioral gate: strong impulsive-risk signal. Consider NO TRADE until the checklist normalizes.")
    elif psych>=5: st.warning("Behavioral warning: elevated FOMO/revenge/fatigue/impulsivity.")
    else: st.success("Behavioral checklist: no high-intensity signal recorded.")
    ...
    if mode=="ANALYSIS ONLY":
        st.info("ANALYSIS ONLY: order execution is disabled by design.")
    elif mode=="PAPER":
        if status=="TRADE REJECTED": st.error("Paper order blocked by critical validation errors. Fix the plan first.")
        elif st.button("Submit PAPER trade",type="primary",key="paper_submit"):
        ...
        st.error("LIVE MODE — REAL MONEY / REAL ORDERS")
        if not live_enabled_by_server(): st.warning("Server-level ENABLE_LIVE_TRADING is OFF. Set it explicitly in Streamlit secrets/environment to permit live execution.")
        okcred,msg=credential_status(exchange_id); st.caption(f"Credentials: {msg}")
        if status=="TRADE REJECTED" or psych>=8:
            st.error("LIVE order blocked by the trade gate.")
        else:
            st.markdown("**Double confirmation required**")
            exact=f"ENABLE LIVE {symbol}"
            phrase=st.text_input(f"Type exactly: {exact}",key="live_phrase")
            understand=st.checkbox("I confirm this is a real leveraged futures order and I reviewed the stop, size, leverage and maximum planned loss.",key="live_ack")
            if st.button("Generate one-time execution code",key="gen_code"):
            ...
            code=st.text_input("Enter one-time code",type="password",key="live_code")
            ...
            if st.button("EXECUTE LIVE ORDER",type="primary",disabled=not ready,key="execute_live"):
                payload_hash=hash_payload(preview)
                if st.session_state.get("last_order_hash")==payload_hash: st.error("Duplicate-order protection: this exact preview was already submitted in this session.")
                else:
                    try:
                        with st.spinner("Submitting protected LIVE order…"):
                        ...
                        if protection.get("tp_errors"): st.warning("Entry stop is attached, but one or more TP ladder actions need review.")
...
def page_positions(exchange_id: str, symbol: str, mode: str):
    st.markdown("## Positions / Execution")
    if mode=="PAPER":
        if st.button("Refresh & process PAPER positions"):
            events=process_paper_positions(exchange_id)
            for e in events: st.info(e)
        p=read_table("paper_positions")
        if p.empty or not len(p[p.status=="OPEN"]): st.info("No open paper positions.")
        else:
            ...
            st.caption("Paper engine uses conservative same-candle ordering: if SL and TP are both touched, SL is processed first.")
    else:
        ok,msg=credential_status(exchange_id)
        if not ok: st.info(f"Private account data unavailable: {msg}"); return
        if st.button("Load private account positions",key="load_live_positions"):
        ...
        if not acct: st.info("Private data is loaded only on request; public analysis does not require API credentials."); return
        ...
        else: st.info("No open positions returned by the exchange.")
        st.caption("For LIVE positions, exchange-reported liquidation price is shown when available and supersedes local estimates.")
    section_label("KILL SWITCH")
    if st.session_state.kill_switch: st.error("KILL SWITCH ACTIVE — NEW ORDERS BLOCKED")
    c1,c2=st.columns(2)
    if c1.button("Activate kill switch",disabled=st.session_state.kill_switch): st.session_state.kill_switch=True; audit("KILL_SWITCH_ON",{"exchange":exchange_id},"CRITICAL"); st.rerun()
    if c2.button("Deactivate kill switch",disabled=not st.session_state.kill_switch): st.session_state.kill_switch=False; audit("KILL_SWITCH_OFF",{"exchange":exchange_id},"WARNING"); st.rerun()
    if mode=="LIVE" and st.session_state.kill_switch:
        st.warning("Optional destructive action: cancel pending orders only. Closing positions is intentionally not automatic.")
        phrase=st.text_input("Type CANCEL PENDING ORDERS",key="cancel_phrase")
        if st.button("Cancel all pending LIVE orders",disabled=phrase!="CANCEL PENDING ORDERS"):
            try: res=cancel_pending_live(exchange_id,None); st.success("Cancel request completed."); st.json(res,expanded=False)
            except Exception as e: error_box("Cancel-all failed",e)

def page_risk_center(exchange_id: str, mode: str):
    st.markdown("## Risk Center / Portfolio Risk")
    limits=get_risk_limits(); capital=st.number_input("Reference account equity (USDT)",min_value=1.0,value=10000.0,step=100.0,key="risk_equity")
    ...
    if ctx["daily_loss_pct"]<=-limits.max_daily_loss_pct: st.error("DAILY RISK LIMIT REACHED — new trades should remain blocked.")
    if ctx["drawdown_pct"]>=limits.max_drawdown_pct: st.error("MAX DRAWDOWN LIMIT REACHED.")
    positions=[]; equity=capital
    if mode=="LIVE" and credential_status(exchange_id)[0]:
        if st.button("Fetch LIVE portfolio risk"):
            try:
    ...
    else: st.info("Portfolio-level live exposure appears here when private positions are loaded. PAPER positions remain visible in Positions.")
    section_label("STRESS TEST")
    notional=st.number_input("Portfolio net directional notional for stress test",min_value=0.0,value=10000.0,step=1000.0)
    stress=[]
    ...
    st.caption("Stress test is linear and hypothetical; gaps, nonlinear liquidation mechanics, correlation breakdown and slippage can worsen outcomes.")
def page_journal_analytics():
    st.markdown("## Journal / Analytics")
    tabs=st.tabs(["Journal","Performance","Reviews","Monte Carlo","Capital Flows"])
    with tabs[0]:
        st.markdown("#### Add / review trade")
        with st.form("journal_form"):
            c1,c2,c3,c4=st.columns(4)
            symbol=c1.text_input("Symbol","BTC/USDT:USDT"); side=c2.selectbox("Side",["LONG","SHORT"]); setup=c3.selectbox("Setup",["Manual","Trend Pullback","Breakout Retest","Range Reversal","Liquidity Sweep","Momentum Continuation"]); lev=c4.number_input("Leverage",1.0,125.0,3.0)
            c1,c2,c3,c4=st.columns(4)
            entry=c1.number_input("Entry",0.0,value=1.0,format="%.8f"); stop=c2.number_input("Stop",0.0,value=.95,format="%.8f"); pnl=c3.number_input("PnL gross",value=0.0); fees=c4.number_input("Fees",min_value=0.0,value=0.0)
            c1,c2,c3,c4=st.columns(4)
            funding=c1.number_input("Funding",value=0.0); mfe=c2.number_input("MFE price units",min_value=0.0,value=0.0); mae=c3.number_input("MAE price units",min_value=0.0,value=0.0); risk_usdt=c4.number_input("Planned risk USDT",min_value=0.0,value=100.0)
            reason=st.text_area("Reason / plan"); lesson=st.text_area("Lesson"); mistake=st.text_input("Mistake / rule broken"); notes=st.text_area("Notes")
            st.caption("Discipline is separate from economic outcome.")
            d1,d2,d3,d4=st.columns(4); respected=d1.checkbox("Respected plan",True); moved=d2.checkbox("Did NOT widen stop",True); chased=d3.checkbox("Did NOT chase",True); within=d4.checkbox("Risk stayed within limit",True)
            save=st.form_submit_button("Save journal trade")
        if save:
            ...
            audit("JOURNAL_TRADE_SAVED",{"id":jid,"symbol":symbol,"net":net}); st.success("Trade saved.")
        j=read_table("journal")
        if j.empty: st.info("No trades yet.")
    ...
    with tabs[1]:
        ...
        if not m.get("trades"): st.info("No completed trades yet.")
        else:
            ...
            st.caption("Sample-size guard: strategy health remains INSUFFICIENT DATA below 10 trades. Positive PnL alone is not treated as proven edge.")
    with tabs[2]:
        j=read_table("journal")
        if j.empty: st.info("No review data.")
        else:
            ...
            for title,start in periods.items():
                ...
                if y.empty: st.caption("No trades in period."); continue
    ...
    with tabs[3]:
        j=read_table("journal")
        if j.empty or pd.to_numeric(j.r_result,errors="coerce").dropna().shape[0]<3: st.info("At least 3 R-results are required; 20+ is preferable.")
        else:
            c1,c2,c3=st.columns(3); sims=c1.slider("Simulations",500,5000,2000,500); future_n=c2.slider("Trades per simulation",10,200,50,10); risk=c3.number_input("Risk % per future trade",.1,10.0,1.0,.1)
            ...
            st.caption("Monte Carlo is a bootstrap of observed R outcomes, not a forecast; regime changes and non-stationarity can invalidate the distribution.")
    with tabs[4]:
        with st.form("capital_flow"):
            c1,c2,c3=st.columns(3); kind=c1.selectbox("Type",["DEPOSIT","WITHDRAWAL"]); amt=c2.number_input("Amount",min_value=0.0,value=0.0); note=c3.text_input("Note"); add=st.form_submit_button("Add capital flow")
        if add and amt>0:
            with db_connect() as con: con.execute("INSERT INTO capital_flows VALUES(?,?,?,?,?)",(uuid.uuid4().hex,iso_now(),kind,amt,note))
            st.success("Capital flow recorded. Deposits/withdrawals are kept separate from trading PnL.")
...
def page_backtest(exchange_id: str, symbol: str):
    ...
    c1,c2,c3,c4=st.columns(4); tf=c1.selectbox("Timeframe",["5m","15m","1h","4h","1d"],index=2,key="bt_tf"); strategy=c2.selectbox("Strategy",["Trend Pullback","Breakout Retest","Range Reversal","Momentum Continuation"]); bars=c3.slider("Bars",300,1500,750,50); risk=c4.number_input("Risk %",.1,5.0,1.0,.1,key="bt_risk")
    c1,c2,c3=st.columns(3); stop_atr=c1.number_input("Stop ATR",.5,5.0,1.5,.1); rr=c2.number_input("Target R",.5,10.0,2.0,.25); fee=c3.number_input("Fee % / side",0.0,.5,.05,.005)
    slip=st.number_input("Slippage % / side",0.0,1.0,.02,.01,key="bt_slip")
    if st.button("Run backtest",type="primary"):
        try:
            with st.spinner("Loading historical candles and simulating conservatively…"):
                ad=ExchangeAdapter(exchange_id); df=ad.fetch_ohlcv(symbol,tf,bars); ad.close(); t,eq,m=run_backtest(df,strategy,risk,stop_atr,rr,fee,slip)
    ...
    if not result: st.info("Backtest uses only available exchange OHLCV. Intrabar collisions are resolved conservatively in favor of the stop."); return
    t,eq,m,df=result
    if not m.get("trades"): st.warning("No trades generated by this rule set in the selected sample."); return
    ...
    st.caption("Parameters are not optimized separately on validation/OOS here; this deliberately reduces the temptation to overfit a small sample.")

def page_playbook_alerts(exchange_id: str, symbol: str):
    st.markdown("## Playbook / Alerts")
    tabs=st.tabs(["Playbook","Alerts","Overtrading / FOMO"])
    with tabs[0]:
        with st.form("playbook_form"):
            name=st.text_input("Setup name"); c1,c2,c3=st.columns(3); side=c1.selectbox("Direction",["BOTH","LONG","SHORT"]); tfs=c2.multiselect("Timeframes",TIMEFRAMES,default=["15m","1h"]); minrr=c3.number_input("Minimum R:R",.5,10.0,1.5,.1)
            c1,c2=st.columns(2); vol=c1.selectbox("Allowed volatility",["ANY","LOW/NORMAL","NORMAL/HIGH","NO EXTREME"]); session=c2.selectbox("Session",["ANY","ASIA","LONDON","NEW YORK","OVERLAPS"])
            cond=st.text_area("Conditions"); inv=st.text_area("Invalidation"); checklist=st.text_area("Checklist",placeholder="One rule per line")
    ...
    with tabs[1]:
        with st.form("alert_form"):
            c1,c2,c3,c4=st.columns(4); kind=c1.selectbox("Alert",["PRICE","RSI","FUNDING","OI_CHANGE","VOLATILITY","SPREAD","STALE_DATA"]); op=c2.selectbox("Operator",[">",">=","<","<="]); threshold=c3.number_input("Threshold",value=0.0,format="%.8f"); sev=c4.selectbox("Severity",SEVERITIES,index=2); note=st.text_input("Note"); add=st.form_submit_button("Add alert")
        if add:
            ...
            st.success("Alert rule saved. This single-file app evaluates rules when the relevant page/app reruns; it is not a background daemon.")
        st.dataframe(read_table("alerts"),use_container_width=True,hide_index=True)
    with tabs[2]:
        j=read_table("journal")
        if j.empty: st.info("No trade history to inspect.")
        else:
            ...
            else: st.success("No simple overtrading/risk-escalation flag detected from stored journal data.")
            st.caption("FOMO/late-entry checks are behavioral/rule-based alerts, not psychological diagnoses.")
...
def page_system_health(exchange_id: str, symbol: str):
    ...
    if snap.get("status")=="OK":
        ...
        rows += [
          {"Source":"Price/Ticker","Status":"CONNECTED" if finite(snap.get("last")) else "ERROR","Freshness sec":0 if finite(snap.get("last")) else np.nan,"Detail":price_fmt(snap.get("last"))},
          {"Source":"Candles","Status":"CONNECTED" if finite(age) and age<300 else "STALE","Freshness sec":age,"Detail":f"{len(snap['df'])} bars"},
    ...
    if snap.get("status")=="OK":
        ...
        if not alerts: st.success("No saved alert rule is currently triggered for this symbol.")
        else:
            for a in alerts:
    ...
    if st.button("Run internal calculation tests"):
        checks=self_checks(); st.dataframe(checks,use_container_width=True,hide_index=True)
        if (checks.Status=="PASS").all(): st.success("All internal calculation self-checks passed.")
        else: st.error("One or more self-checks failed. Do not use LIVE mode until resolved.")
    section_label("AUDIT LOG")
    log=read_table("audit")
    if log.empty: st.info("Audit log is empty.")
    else: st.dataframe(log.sort_values("id",ascending=False).head(200),use_container_width=True,hide_index=True)
    st.caption("The app uses retry with exponential backoff, API timeouts and Streamlit caches. A failing data source degrades its module instead of fabricating replacement market data.")

def page_configuration(exchange_id: str, symbol: str):
    st.markdown("## Configuration / Setup / Deployment")
    tabs=st.tabs(["Risk Limits","Trading Safety","API & Secrets","Deployment","Architecture"])
    with tabs[0]:
        lim=get_risk_limits()
        with st.form("risk_limits_form"):
            c1,c2,c3=st.columns(3)
            mr=c1.number_input("Max Risk / Trade %",.1,100.0,lim.max_risk_trade_pct,.1); dl=c2.number_input("Max Daily Loss %",.1,100.0,lim.max_daily_loss_pct,.1); wl=c3.number_input("Max Weekly Loss %",.1,100.0,lim.max_weekly_loss_pct,.1)
            c1,c2,c3=st.columns(3)
            ml=c1.number_input("Max Monthly Loss %",.1,100.0,lim.max_monthly_loss_pct,.1); op=c2.number_input("Max Open Risk %",.1,100.0,lim.max_open_risk_pct,.1); exp=c3.number_input("Max Portfolio Exposure %",10.0,2000.0,lim.max_portfolio_exposure_pct,10.0)
            c1,c2,c3=st.columns(3)
            mu=c1.number_input("Max Margin Usage %",1.0,100.0,lim.max_margin_usage_pct,1.0); lev=c2.number_input("Max Leverage",1.0,125.0,lim.max_leverage,1.0); dd=c3.number_input("Max Drawdown %",1.0,100.0,lim.max_drawdown_pct,1.0)
            c1,c2,c3=st.columns(3)
            ct=c1.number_input("Max Concurrent Trades",1,50,lim.max_concurrent_trades,1); corr=c2.number_input("Max Correlated Trades",1,20,lim.max_correlated_trades,1); rr=c3.number_input("Minimum R:R",.1,20.0,lim.min_rr,.1)
            save=st.form_submit_button("Save risk policy")
        if save:
            save_setting("risk_limits",asdict(RiskLimits(mr,dl,wl,ml,op,exp,mu,lev,dd,int(ct),int(corr),rr))); st.success("Risk policy saved locally.")
    with tabs[1]:
        st.markdown("### Safety hierarchy")
        st.markdown("1. Preserve capital  \n2. Limit risk  \n3. Prevent operational errors  \n4. Prevent impulsive execution  \n5. Execute correctly  \n6. Analyze opportunities  \n7. Seek returns")
        st.info("LIVE is OFF by default and additionally requires the server secret `ENABLE_LIVE_TRADING=true`, configured credentials, an approved/conditional gate, an attached-stop capability, a typed phrase and a one-time code.")
        st.warning("Use exchange API keys with withdrawals disabled and only the minimum futures permissions required. IP restrictions are recommended where the exchange/account supports them.")
        st.caption("The emergency kill switch blocks new orders. Cancel-all requires a separate typed confirmation. It never closes positions automatically.")
    with tabs[2]:
...
OKX_API_PASSPHRASE = "..."
''',language="toml")
        st.caption("Secrets are read from environment variables first, then `st.secrets`. They are never rendered, logged or included in the audit payload.")
    with tabs[3]:
...
tzdata>=2025.2""",language="text")
        ...
        st.warning("SQLite on Streamlit Community Cloud should be treated as ephemeral local persistence. For durable multi-device history, migrate the persistence functions to a durable external store while keeping the adapter inside this same `app.py` if the one-file constraint remains.")
    with tabs[4]:
        st.markdown("### Internal one-file architecture")
...
MAIN APP""",language="text")
        st.caption("All financial scores are rule-based and explainable. Data confidence is distinct from signal strength. Missing data remains missing instead of being invented.")

def sidebar() -> Tuple[str,str,str,str]:
    with st.sidebar:
        st.markdown(f"### ⚡ {APP_NAME}")
        mode=st.selectbox("OPERATING MODE",["ANALYSIS ONLY","PAPER","LIVE"],index=["ANALYSIS ONLY","PAPER","LIVE"].index(st.session_state.mode),key="mode_select")
        st.session_state.mode=mode
        exchange_id=st.selectbox("Exchange",list(SUPPORTED_EXCHANGES),format_func=lambda x:SUPPORTED_EXCHANGES[x],index=list(SUPPORTED_EXCHANGES).index(st.session_state.exchange_id) if st.session_state.exchange_id in SUPPORTED_EXCHANGES else 0)
        ...
        try: symbols=cached_market_symbols(exchange_id)
            ...
            if info["code"]=="HOST_REGION_RESTRICTED":
                st.warning("Exchange blocked from this app server region. The app is online; the data endpoint is refusing the host.")
            else:
        ...
        if symbols:
            ...
            symbol=st.selectbox("Contract",symbols,index=symbols.index(default))
        else:
            symbol=st.text_input("Contract",st.session_state.symbol or DEFAULT_SYMBOL)
        st.session_state.symbol=symbol
        tf=st.selectbox("Primary timeframe",TIMEFRAMES,index=TIMEFRAMES.index(st.session_state.timeframe) if st.session_state.timeframe in TIMEFRAMES else 3)
        st.session_state.timeframe=tf
        st.markdown("---")
        nav=st.radio("Navigation",["Command Center","Market / Analysis","Scanner","Trade Builder","Positions / Execution","Risk Center","Journal / Analytics","Backtest","Playbook / Alerts","Data Quality / Health","Configuration"],key="nav_radio")
        st.markdown("---")
        if mode=="LIVE": st.error("LIVE · REAL ORDERS")
        elif mode=="PAPER": st.warning("PAPER · SIMULATION")
        else: st.info("ANALYSIS ONLY")
        st.checkbox("Debug diagnostics",key="debug_mode")
...
def main():
    ...
    if ccxt is None:
        st.error("CCXT is not installed. Install requirements before using market data.")
    ...
    st.markdown("---")
    st.caption("Futures Command Center Pro is an analysis and risk-management tool, not a guarantee of results. Leveraged crypto futures can cause rapid and significant losses. Prefer NO TRADE whenever data, risk, setup quality or discipline is inadequate.")
