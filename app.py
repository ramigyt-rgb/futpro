...
def market_snapshot(exchange_id: str, symbol: str, timeframe: str="15m") -> Dict[str,Any]:
    started=time.perf_counter(); errors=[]; ticker={}; ob={}; df=pd.DataFrame(); deriv={}
    ...
    except Exception as e: errors.append(f"ohlcv: {e}")
    try: deriv=derivatives_engine(exchange_id,symbol,df)
    except Exception as e: deriv={"status":"UNAVAILABLE","error":str(e)}; errors.append(f"derivatives: {e}")
    if df.empty:
        return {"symbol":symbol,"ticker":ticker,"orderbook":ob,"df":df,"derivatives":deriv,"errors":errors,"status":"UNAVAILABLE","latency_ms":(time.perf_counter()-started)*1000}
...
def page_command_center(exchange_id: str, symbol: str):
    ...
    try:
        ...
        if snap["status"]!="OK":
            ...
            if snap.get("errors"): st.caption(" · ".join(snap["errors"]))
            return
...
def sidebar() -> Tuple[str,str,str,str]:
    with st.sidebar:
        ...
        except Exception as e: st.caption(f"Market list unavailable: {type(e).__name__}")
        if symbols:
