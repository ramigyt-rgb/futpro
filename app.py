...
[data-testid="stSidebar"]{background:linear-gradient(180deg,#07101a,#09131f 58%,#07101a);border-right:1px solid var(--line);min-width:285px}
[data-testid="stSidebar"]>div:first-child{padding-top:max(.65rem,env(safe-area-inset-top));padding-bottom:calc(1rem + env(safe-area-inset-bottom))}
[data-testid="stSidebar"] *{color:#deebf7}
[data-testid="stSidebar"] label,[data-testid="stSidebar"] p,[data-testid="stSidebar"] span{color:#deebf7!important}
[data-testid="stSidebar"] [data-baseweb="select"]>div,[data-testid="stSidebar"] input{background:#101d2b!important;border-color:#2b405b!important;color:#fff!important}
[data-testid="stSidebar"] [role="radiogroup"] label{border-radius:11px;padding:.42rem .5rem;margin:.08rem 0;min-height:42px}
[data-testid="stSidebar"] [role="radiogroup"] label:hover{background:#102033}
...
div[data-testid="stButton"]:has(button#app_sidebar_toggle),button[aria-label*="MENÚ"],button[aria-label*="MENU"]{z-index:2147483000}
...
@media (max-width:1180px){
 ...
 [data-testid="column"]{min-width:190px!important;flex:1 1 220px!important}
...
def render_sidebar_toggle() -> None:
    """App-owned sidebar visibility that remains recoverable on iOS/iPad/Android/desktop."""
    visible=bool(st.session_state.get("sidebar_visible",True))
    if not visible:
        st.markdown("""<style>[data-testid='stSidebar']{display:none!important;width:0!important;min-width:0!important}[data-testid='stAppViewContainer']>.main{width:100%!important;max-width:100%!important;flex:1 1 100%!important}</style>""",unsafe_allow_html=True)
    label=("◀  OCULTAR MENÚ" if visible else "☰  MENÚ") if st.session_state.get("language","es")=="es" else ("◀  HIDE MENU" if visible else "☰  MENU")
    if st.button(label,key="app_sidebar_toggle",help="Mostrar u ocultar el panel lateral"):
        st.session_state.sidebar_visible=not visible; st.rerun()

def sidebar() -> Tuple[str,str,str,str]:
    with st.sidebar:
        st.markdown(f"### ⚡ {APP_NAME}")
        lang_codes=list(LANGUAGES.keys()); language=st.selectbox("🌐 Idioma / Language",lang_codes,format_func=lambda x:LANGUAGES[x],index=lang_codes.index(st.session_state.get("language","es")),key="language_select"); st.session_state.language=language
        mode=st.selectbox(tr("OPERATING MODE"),["ANALYSIS ONLY","PAPER","LIVE"],format_func=tr,index=["ANALYSIS ONLY","PAPER","LIVE"].index(st.session_state.mode),key="mode_select"); st.session_state.mode=mode
        exchange_id=st.selectbox(tr("Exchange"),list(SUPPORTED_EXCHANGES),format_func=lambda x:SUPPORTED_EXCHANGES[x],index=list(SUPPORTED_EXCHANGES).index(st.session_state.exchange_id) if st.session_state.exchange_id in SUPPORTED_EXCHANGES else 0); st.session_state.exchange_id=exchange_id
        symbols=[]
        try: symbols=cached_market_symbols(exchange_id)
        except Exception as e:
            info=classify_exchange_error(e); st.warning(tr("Exchange blocked from this app server region. The app is online; the data endpoint is refusing the host.")) if info["code"]=="HOST_REGION_RESTRICTED" else st.caption(f"Market list unavailable: {info['code']}")
        if symbols:
            default=st.session_state.symbol if st.session_state.symbol in symbols else next((s for s in symbols if s.startswith("BTC/USDT")),symbols[0]); symbol=st.selectbox(tr("Contract"),symbols,index=symbols.index(default))
        else: symbol=st.text_input(tr("Contract"),st.session_state.symbol or DEFAULT_SYMBOL)
        st.session_state.symbol=symbol
        if exchange_id=="binanceusdm": st.caption(("📡 Datos públicos: Binance Futures real." if _binance_futures_public_available() else "📡 Fallback: Binance Vision Spot proxy porque Binance Futures público no responde desde este host. LIVE siempre revalida contra Futures real.") if st.session_state.get("language","es")=="es" else ("📡 Public data: real Binance Futures." if _binance_futures_public_available() else "📡 Fallback: Binance Vision Spot proxy because Binance Futures public is unavailable from this host. LIVE always revalidates against real Futures."))
        tf=st.selectbox(tr("Primary timeframe"),TIMEFRAMES,index=TIMEFRAMES.index(st.session_state.timeframe) if st.session_state.timeframe in TIMEFRAMES else 3); st.session_state.timeframe=tf
        density=st.segmented_control("Vista / View",["FOCUS","PRO"],default=st.session_state.get("view_density","PRO"),key="density_control") if hasattr(st,"segmented_control") else st.selectbox("Vista / View",["FOCUS","PRO"],index=1)
        if density: st.session_state.view_density=density
        st.markdown("---"); nav=st.radio(tr("Navigation"),["Command Center","Market / Analysis","Scanner","Trade Builder","Positions / Execution","Risk Center","Journal / Analytics","Backtest","Playbook / Alerts","Data Quality / Health","Configuration"],format_func=tr,key="nav_radio"); st.markdown("---")
        if mode=="LIVE": st.error(tr("LIVE · REAL ORDERS"))
        elif mode=="PAPER": st.warning(tr("PAPER · SIMULATION"))
        else: st.info(tr("ANALYSIS ONLY"))
        if st.session_state.get("kill_switch"): st.error("⛔ KILL SWITCH ACTIVE")
        st.checkbox(tr("Debug diagnostics"),key="debug_mode"); st.caption(f"v{APP_VERSION} · build {BUILD}")
